...
class DualBrowserEvaluator:
    ...
    def print_summary(self):
        ...
        print(f"\n📈 ÜBERSICHT")
        print(f"   URLs analysiert: {len(self.results)}")
        ...
        urls_with_bugs = sum(1 for r in self.results if r.confirmed_bugs > 0)
        ...
        # Per-URL table
        print(f"\n{'='*70}")
        print(f"{'URL':<40} {'Coverage':>10} {'Flows':>8} {'Bugs':>6} {'Zeit':>8}")
        print(f"{'-'*70}")
        
        for r in self.results:
            url_short = r.url[:38] + '..' if len(r.url) > 40 else r.url
            cov = f"{r.coverage.coverage_percent:.1f}%" if r.coverage else "N/A"
            print(f"{url_short:<40} {cov:>10} {r.taint_flows:>8} {r.confirmed_bugs:>6} {r.total_duration:>7.1f}s")
